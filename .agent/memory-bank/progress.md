...
- Create collections (idempotent via `agent_tool.py create-collection`).
...
- Agent-friendly JSON CLI (`agent_tool.py`) with on-disk search cache → tiny
  two-step workflow (`search` then `add` by TMDB id alone).
- `bulk-add` single-pass JSON-driven importer with year/original-title matching.
